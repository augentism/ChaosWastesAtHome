-- The vote screen: the launcher's layout, without the launcher's controls.
--
-- Built from launch_view_definitions rather than the end-of-round picker's,
-- because this is a screen you stop and read rather than a prompt you answer --
-- so it gets the big cards that show every modifier and what it does, instead of
-- the compact ones that reveal a description only on hover.
--
-- What is deliberately absent: the difficulty slider (the run's difficulty is
-- already decided), Reroll and Begin (nobody here is starting anything), and
-- both strips (a loadout is not changeable mid-mission).
...
-- Higher than the launcher's 330: there is no difficulty slider above the
-- cards here, so the row moves up into the space that leaves.
local CARDS_TOP = 270
...
-- The card, the launcher's own, so the three screens that offer missions all
-- look like one screen -- they are offering the same thing at different moments.
--
-- Details are drawn on every card at once rather than in a hover panel. The
-- end-of-round picker hovers because it is squeezed in beside the scoreboard;
-- here there is room for all three, and reading them side by side is the whole
-- point of a vote.
local function _option_widget(scenegraph_id)
	return UIWidget.create_definition({
		...
		-- The tally, over the top-right of the preview.
		--
		-- On the card rather than in a list beside it: the thing being counted
		-- is the card, and a separate tally column makes you look twice to work
		-- out which number belongs to which mission.
		{
			pass_type = "text",
			value_id = "votes",
			style = {
	...
	title = UIWidget.create_definition({
		{
			pass_type = "text",
			value_id = "text",
			value = mod:localize("vote_view_title"),
			style = {
				font_type = "proxima_nova_bold",
				font_size = 34,
